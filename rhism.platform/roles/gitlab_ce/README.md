...
Installs and manages [GitLab Community Edition](https://about.gitlab.com/install/) (the
omnibus package) on Enterprise Linux 9, covering the full lifecycle: install/remove,
`gitlab.rb` configuration, CI runner registration, backup and restore, package upgrade,
...
`scm_deploy` dispatcher** when `scm_type: gitlab_ce`. Both editions consume the **same
...
Community and Enterprise are distinct install targets a user picks exactly one of — EE
additionally requires a Red Hat subscription (RHSM).

## Requirements

- Ansible: 2.15+ · Collections: `ansible.posix` (firewalld), `community.general`
- Target: an EL9 host with `dnf` and internet access to `packages.gitlab.com`
- Privilege escalation (`become`) for package, firewall, and `gitlab-ctl` operations

## Role Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `action` | `present` | `present`/`absent` · `configure`/`reconfigure` · `runners` · `backup`/`restore` · `upgrade` · `status` · `started`/`stopped`/`restarted`. Shared with `gitlab_ee`; the dispatcher passes it in. |
...
| `scm_rhsm_skip_registration` | `false` | Interface parity with `gitlab_ee`; unused by CE. |

## Use Cases

**Standalone — install GitLab CE on a host:**
...
**Standalone — remove GitLab CE (and purge config):**
...
```bash
cd roles/gitlab_ce && molecule test   # standalone, independent of any dispatcher
```
...
and the `scm_upgrade_execute` dry-run gate — without installing a real GitLab. A live
install is exercised in the full-stack test lab (`inventories/test/`), not in molecule.
...
- Depth doc: [`docs/gitlab_ce.md`](../../docs/gitlab_ce.md) — internals, action workflows,
